# Program: fit_lasso.py
# Purpose: runs the joint estimation procedure and uses lasso (not measurement error-aware)
...
for i in range(len(noise_dim_list)):
    ...
    for w in range(len(lambda1_list)):
        ...
        with open(str('sim_glasso' + "_noiseq" + str(noise_dim) +'-' +  str(q) + "_lambda" + str(lamb) + "_" + str(optimizer)+'_' + str(date) + '_' + str(seed_num) +'.pkl'), 'wb') as file: 
