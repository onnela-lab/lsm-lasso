# Program: fit_lasso_ME.py
# Purpose: runs the joint estimation procedure and uses measurement-aware lasso
...
data_date = "2025-03-27" # date the simulation data was created
...
eta = 10
T = 100000
noise_dim_list = [sys.argv[3]]#[0, 10, 20]
...
for i in range(len(noise_dim_list)):
    ...
    for w in range(len(lambda1_list)):
        lamb = lambda1_list[w]
        print("Current noise level: ", noise_dim)
        print("Current lambda value: ", lamb)
        
        #lambda_grid = np.geomspace(1, 1e-3, 15)
        lambda_grid = np.linspace(0.3,1e-2, 10)
        
        print("Searched lasso penalties: ", lambda_grid)
        
        ## Group lasso
        res_glas = run_simulations(n=n, k=k, q=q, eta=eta, lamb=lamb, T=T,
                         A_in=A_in, Y_in=Y_in,
                         ...
                         lambda_grid=lambda_grid, cv_indicate = False, cv_folds=0,
                         se_rule = 0, ME_aware = True, true_avail = True, Z_true = Z_in, B_true = B_in,
                         random_state=0, optimizer = str(optimizer),
                         verbose=True)

        with open(str('sim_MEglasso' + "_noiseq" + str(noise_dim) +'-' +  str(q) + "_lambda" + str(lamb) + "_" + str(optimizer)+'_' + str(date) + '_' + str(seed_num) +'.pkl'), 'wb') as file: 
