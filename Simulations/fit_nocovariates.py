# Program: fit_nocovariates.py
# Purpose: runs the joint estimation procedure without using node covariate information
...
for i in range(len(noise_dim_list)):
    ...
    res = run_simulations(n=n, k=k, q=q, eta=eta, lamb=lamb, T=T,
                         A_in=A_in, Y_in=Y_in,
    ...
    with open(str('sim_nocov' + "_noiseq" + str(noise_dim) +'-' +  str(q) + "_" + str(optimizer) + "_" + str(date) + '_' + str(seed_num) +'.pkl'), 'wb') as file: 
